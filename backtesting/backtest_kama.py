...
def kama_main(param):
    ...
    data[f"KAMA{param['period']}"] = talib.KAMA(data['Close'], timeperiod=param['period'])
    ...
    try:
        ...
        data['Cross_Close'] = 0
        data.loc[(data[f"KAMA{param['period']}"].shift(2) < data['Close'].shift(2)) & (data[f"KAMA{param['period']}"].shift(1) > data['Close'].shift(1)), 'Cross_Close'] = -1
        data.loc[(data[f"KAMA{param['period']}"].shift(2) > data['Close'].shift(2)) & (data[f"KAMA{param['period']}"].shift(1) < data['Close'].shift(1)), 'Cross_Close'] = 1

    data['Trend'], data['Cross'], data['Signal'], data['Position'], data['aPosition'] = 0, 0, 0, 0, 0

    data.loc[(data[f"KAMA{param['period']}"].shift(1) > data[f"MA{param['period_ma']}"].shift(1)), 'Trend'] = 1
    data.loc[(data[f"KAMA{param['period']}"].shift(1) < data[f"MA{param['period_ma']}"].shift(1)), 'Trend'] = -1

    data.loc[(data[f"KAMA{param['period']}"].shift(2) < data[f"MA{param['period_ma']}"].shift(2)) & (data[f"KAMA{param['period']}"].shift(1) > data[f"MA{param['period_ma']}"].shift(1)), 'Cross'] = 1
    data.loc[(data[f"KAMA{param['period']}"].shift(2) > data[f"MA{param['period_ma']}"].shift(2)) & (data[f"KAMA{param['period']}"].shift(1) < data[f"MA{param['period_ma']}"].shift(1)), 'Cross'] = -1

    data.loc[(data['Trend'] == 1) & (data['Cross'] != 0) & (data['Close'].shift(1) < data[f"KAMA{param['period']}"].shift(1)), 'Signal'] = 1
    data.loc[(data['Trend'] == -1) & (data['Cross'] != 0) & (data['Close'].shift(1) > data[f"KAMA{param['period']}"].shift(1)), 'Signal'] = -1

    data['PnL'], data['Cum_PnL'], data['Trade_PoL'], data['Period'] = 0, 0, 0, 0
    data[f"D_CLr{param['cl_ratio']}"] = 0
    
    try:
        data[f"D_TPr{param['tp_ratio']}"] = 0
    except:
        pass
    ...
    if not param['trading_mode']:
        ...
        if param['batch_run'][0]:
            ...
            if param['trailing_stop']:
                ...
                extend_com_param = ''
            try:
                com_param = f"KAMA{param['period']}_MA{param['period_ma']}_CLr{param['cl_ratio']}_TPr{param['tp_ratio']}_De{param['decay']}_Size{param['trading_size']}{extend_com_param}"
            except:
                com_param = f"KAMA{param['period']}_MA{param['period_ma']}_CLr{param['cl_ratio']}_CrossTP_De{param['decay']}_Size{param['trading_size']}{extend_com_param}"
...
def kama_backtest(kwargs):
    ...
    if param['batch_run'][0]:
        ...
        with concurrent.futures.ProcessPoolExecutor() as executor:
            ...
            batch_results = [executor.submit(kama_main, prod) for prod in prods]
        ...
        if not param['trading_mode']:
            data, data_dir = kama_main(param)
        else:
            data = kama_main(param)
            param['show_plt'] = False
