...
def adx_main(param):
    ...
    data[f"ADX{param['period']}"] =  talib.ADX(data['High'], data['Low'], data['Close'], param['period'])
    data[f"ADX{param['period']}_RO"] = data[f"ADX{param['period']}"].rolling(param['period_ro']).mean()
    ...
    data['Trend'], data['Signal'], data['Position'], data['aPosition'] = 0, 0, 0, 0

    data.loc[(data['Close'].shift(1) / data[f"MA{param['period_ma']}"].shift(1)) > 1.005, 'Trend'] = 1
    data.loc[(data['Close'].shift(1) / data[f"MA{param['period_ma']}"].shift(1)) < 0.995, 'Trend'] = -1

    data.loc[(data['Trend'] > 0) & (data[f"ADX{param['period']}"].shift(1) > data[f"ADX{param['period']}_RO"].shift(1)) & (data[f"ADX{param['period']}_RO"].shift(1) < param['threshold']) & (data[f"ADX{param['period']}"].shift(1) > param['threshold']), 'Signal'] = 1
    data.loc[(data['Trend'] < 0) & (data[f"ADX{param['period']}"].shift(1) > data[f"ADX{param['period']}_RO"].shift(1)) & (data[f"ADX{param['period']}_RO"].shift(1) < param['threshold']) & (data[f"ADX{param['period']}"].shift(1) > param['threshold']), 'Signal'] = -1
    ...
    if not param['trading_mode']:
        ...
        if param['batch_run'][0]:
            ...
            if param['trailing_stop']:
                ...
                extend_com_param = ''
            com_param = f"ADX{param['period']}_MA{param['period_ma']}_RO{param['period_ro']}_TH{param['threshold']}_CLr{param['cl_ratio']}_TPr{param['tp_ratio']}_De{param['decay']}_Size{param['trading_size']}{extend_com_param}"
...
def adx_backtest(kwargs):
    ...
    if param['batch_run'][0]:
        ...
        with concurrent.futures.ProcessPoolExecutor() as executor:
            ...
            batch_results = [executor.submit(adx_main, prod) for prod in prods]
    ...
    if param['show_plt']:
        show_plt(data=data, data_dir=data_dir, down_sampling=False, **param)
